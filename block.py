...
class Block(object):
    ...
    def header_string(self):
        return (str(self.index) + self.prev_hash + self.data + str(self.timestamp) + str(self.nonce)).encode('utf-8')

    def generate_header(index, prev_hash, data, timestamp, nonce):
        return (str(index) + prev_hash + data + str(timestamp) + str(nonce)).encode('utf-8')
